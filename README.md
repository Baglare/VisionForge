...
Trial panelinde Donma, Ateş ve Kalkan mühürleri ayrı göstergeler olarak görünür. Açılmamış mühürler soluk, açılmış mühürler parlak, sıradaki mühür ise hafif vurgulu çizilir.
...
### Guided Demo Mode / Demo Rehberi

Portfolyo sunumu için `G` ile Demo Rehberi başlatılabilir. Rehber, mevcut özelliklerin yerine geçmez; sadece sıradaki demo adımını küçük bir panelde gösterir. `N` sonraki adıma, `P` önceki adıma geçer, `G` rehberi kapatır. Panelin altındaki küçük kısayol alanında `Q`, `E`, `B`, `T`, `Esc`, `N/P`, `H` ve `D` hatırlatmaları görünür.

Demo sırası: Kamera ve profil, Doğrulama, Büyü Kitabı, Donma, Ateş, Kalkan, Trial Mode, Trial tamamlama, Final. İlk adımda kayıtlı kullanıcı yoksa `E` ile kayıt başlatılabileceği belirtilir. Büyü Kitabı adımında `B` ve sağ/sol ok ipuçları ayrıca gösterilir. Otomatik tamamlanan adımlar en az kısa bir süre görünür kalır; manuel ilerleme için `N` her zaman hemen çalışır.
